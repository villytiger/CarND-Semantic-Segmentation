...
def layers(vgg_layer3, vgg_layer4, vgg_layer7, num_classes):
    ...
    """
    vgg_layer3 = tf.layers.conv2d(vgg_layer3, num_classes, 1, padding="same",
                                  kernel_initializer=tf.truncated_normal_initializer(
                                      stddev=0.01),
                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    vgg_layer4 = tf.layers.conv2d(vgg_layer4, num_classes, 1, padding="same",
                                  kernel_initializer=tf.truncated_normal_initializer(
                                      stddev=0.01),
                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    vgg_layer7 = tf.layers.conv2d(vgg_layer7, num_classes, 1, padding="same",
                                  kernel_initializer=tf.truncated_normal_initializer(
                                      stddev=0.01),
                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    input = tf.layers.conv2d_transpose(vgg_layer7, num_classes, 4, 2, padding="same",
                                       kernel_initializer=tf.truncated_normal_initializer(
                                           stddev=0.01),
                                       kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    input = tf.add(input, vgg_layer4)

    input = tf.layers.conv2d_transpose(input, num_classes, 4, 2, padding="same",
                                       kernel_initializer=tf.truncated_normal_initializer(
                                           stddev=0.01),
                                       kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    input = tf.add(input, vgg_layer3)
